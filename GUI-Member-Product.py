...
################### Function เรียกใช้ฟังก์ชันจากไฟล์อื่น ################3
from menufunction import *
addproduct = Addproduct() #เรียก class Addmember จากข้างนอกเข้ามา
product_icon = ProductIcon() # เรียก class ProductIcon
...
#------------------------------------------------
#product Menu
productmenu = Menu(menubar,tearoff=0) # ใส่ไว้ใน menubar ปิดการ tearoff ของเมนู
menubar.add_cascade(label ='Product',menu=productmenu) # add 

productmenu.add_command(label='Add Product',command=addproduct.command) # ดึงฟังก์ชันมาจาก class AddMember


#---------------------------------
# Setting

settingmenu = Menu(menubar,tearoff=0) # ใส่ไว้ใน menubar ปิดการ tearoff ของเมนู
menubar.add_cascade(label ='Setting',menu=settingmenu)# add 

settingmenu.add_command(label='Product Icon',command = product_icon.command) # ดึงฟังก์ชันมาจาก class AddMember
...
'''
concept ของการทำปุ่มเพ่ิ่มไอคอน เวลามีการแอดปุ่มแล้วให้อัพเดทใหม่ คือเราต้องสร้างดิก เก็บข้อมูลทุกปุ่มลงไปในดิก
จากนั้น เมื่อทำการแอดปุ่ม จะต้องมีการลบค่าเดิมที่เคยเก็บไว้ในดิกออก และเขียนค่าใหม่ทับลงไปทั้งหมด 
แล้วค่อยแสดงผลอีกครั้ง 
'''
...
# ส่งค่า dict ของปุ่ม
addproduct.button_list = button_dict #เรียกมากจาก class ใน menufunction.py ประกาศให้เป็นดิกชื่อ butะon_dict ที่เราสร้างไว้
addproduct.button_frame = CF1 # เรียกมาจาก class แยกเหมือนกัน ประกาศให้มันกลายเป็นเฟรมชื่อ CF1 ที่เราเอาปุ่มไปแปะ

# ส่งค่า dict ของปุ่มเฟรมไปหาหน้าต่างที่ต้องการอัพเดท
product_icon.button_list = button_dict #เรียกมากจาก class ใน menufunction.py ประกาศให้เป็นดิกชื่อ butะon_dict ที่เราสร้างไว้
product_icon.button_frame = CF1 # เรียกมาจาก cl
product_icon.function_add = AddMenu # สั่งว่าบรรทัดนี้คือฟังก์ชัน Addmenu

print(button_dict)

def testclearbutton(event): # ฟังก์ชันเพื่อเคลียร์ปุ่มกดทั้งหมด
    for b in button_dict.values(): # for loop เฉพาะส่วน value
        # b = {'button':B, 'row':row, 'column':column} 
        b['button'].grid_forget() # ทำให้ปุ่ม index ที่ 1 ที่เก็บในดิก หายไป b มีค่าอ้างอิงจากบรรทัดบน

GUI.bind('<F5>',testclearbutton) # กด F5 จะทำการลบปุ่มทั้งหมดทุกปุ่ม
...
v_databasecode = IntVar() # ไว้เก็บตัวเลขฐานข้อมูลที่มีการเลือก
...
def EditMember(): # เมื่อมีการ double click จะปิดปุ่มบันทึก
    ...
    code = v_databasecode.get() #f ดึงรหัสฐานข้อมูลที่เลือก
    allmember[code][2] = v_fullname.get() #append ค่าใส่เข้าไปใน ดิก allmemeber โดย key = รหัสสมาชิก(code)+ชื่อ-สกุล(index 1) ,value = v_fullname.get
    ...
    Update_member(code,'points',v_points.get()) # points มี s เพราะมาจาก data base
...
header = ['ID','Code', 'ชื่อ-สกุล', 'เบอร์โทร','ประเภทสมาชิก','คะแนนสะสม'] #สร้างหัวข้อตาราง อันนี้คิดว่าน่าจะเอาไว้กำหนดจำนวนคอลัมน์ แต่ไม่ได้ใช้สำหรับแสดงผลข้อความที่เป็นตัวอักษร เช่นอันนี้มี 5 ช่อง
hwidth = [50,50,200,100,100,100] #สร้างลิสต์เอาไว้เพื่อกำหนดความกว้างของชอ่ง หน่วนเป็น pixel
...
def DeleteMember(event=None):
    ...
    if choice == True:
        ...
        if len(select)!= 0:
            ...
            Delete_member(data[0]) # ดึง function มาจาก database
...
def UpdateMemberInfo(event=None):
    ...
    v_fullname.set(memberinfo[2]) #ถูกset ให้แสดงค่าใน index ที่ 1 ของ allmember ซึ่งก็คือ ชื่อ-สกุล เอาไปแปะไว้ตรงช่อง entry ชื่อ-สกุล เพื่อให้เราไปลบค่าได้
...
def UpdateTable_Member():
    ...
    #with open('member.csv',newline='',encoding='utf-8') as file: # เปิดไฟล์ CSV
    #fr = csv.reader(file) # file reader อ่านไฟล์
    ...
    for row in fr:
        ...
        allmember[code] = list(row) # ต้องแปลง tuple เป็น list เพราะ tuple แก้ไขข้อมูลไม่ได้
    ...
    last_member = row[1] #แทนข้อมูล index 1 ของ row ใน database (row แบบบรรทัดล่าสุดด้วยนะ) เป็นการประกาศตัวแปรว่าให้ last_member แทนข้อความของ row ในตำแหน่ง index 0 ในที่นี้คือคำว่า M-1001
    # M-1001
